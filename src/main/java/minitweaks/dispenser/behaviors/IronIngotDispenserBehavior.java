...
public class IronIngotDispenserBehavior extends OptionalDispenseItemBehavior {
    protected ItemStack execute(BlockSource pointer, ItemStack stack) {
        ...
        List<IronGolem> list = pointer.level().getEntities(EntityType.IRON_GOLEM, new AABB(blockPos), EntitySelector.LIVING_ENTITY_STILL_ALIVE.and((entity) -> {
        ...
        if(!list.isEmpty()) {
            // get random golem
            IronGolem ironGolem = Util.getRandom(list, pointer.level().getRandom());
            // heal golem
